      ...
      if (s != "") {
        ...
        if (event == "telemetry") {
          // j[1] is the data JSON object
			...
			// TODO: use the previous list of path points : helps smoothening of path 
...
//							}
			...
			// create a list of widely spaced (x,y) waypoints, spaced evenly at 30m
			// Then interpolate these waypoints using the spline function and fill it with more points
			vector<double> ptsx;
			vector<double> ptsy;

			// use current state of the vehicle as reference state : x,y and Yaw
			// keep track of either where the car is at or previous path's end point
			double ref_x = car_x;
			double ref_y = car_y;
